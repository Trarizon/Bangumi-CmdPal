...
internal sealed partial class SubjectListItem : ListItem
{
    ...
    private readonly AuthorizableBangumiClient _client;
    private readonly SearchResponsedSubject _subject;
    private readonly CancellationToken _cancellationToken;

    public SubjectListItem(AuthorizableBangumiClient client, SearchResponsedSubject subject, CancellationToken cancellationToken)
    {
        _client = client;
        _subject = subject;
    ...
    {
        var self = await _client.GetSelfAsync(cancellationToken).ConfigureAwait(false);
        if (self is null)
            return;
